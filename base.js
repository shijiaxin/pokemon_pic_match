...
point.prototype.draw=function(length){
	...
	context.drawImage(imageArray[0],column*96,row*96,96,96,
			this.x*length,this.y*length,length,length);
};
...
imageArray[0].onload=function()
{
	...
	if(gameboard.best!=null && gameboard.best>0){
		if(gameboard.best>100)
			gameboard.bartimerid=setInterval(function(){updatebar()},1000);
		else
			gameboard.bartimerid=setInterval(function(){updatebar()},100);
	}	 
}
